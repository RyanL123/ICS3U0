...
def choose_ancestry(stats):
    ...
    logging.debug("Starting choose ancestry")
    ...
    if choice == 1:
        ...
        free_boost(stats, boost_options)
        free_boost(stats, boost_options)
    logging.debug("Chosen ancestry: %s", ancestries[choice-1])
...
def choose_heritage(ancestry):
    ...
    """
    logging.debug("Starting choose heritage")
    dwarf = [
...
def choose_background():
    ...
    """
    logging.debug("Starting choose background")
    backgrounds = [
        "Academic",
        "Acolyte",
        "Acrobat",
    ...
    for i in range(len(uppercase_list)):
        if uppercase_list[i] == choice.replace(" ", "").upper():
            index = i
            break
    logging.debug("Chosen background: %s", backgrounds[index])
...
def choose_class():
    ...
    """
    logging.debug("Starting choose class")
    classes = [
        "Alchemist",
        "Barbarian",
        "Bard",
    ...
    while not (1 <= choice <= 12):
        try:
            ...
            print("Please input an integer between 1 - 12")
    logging.debug("Chosen class: %s", classes[choice-1])
...
def choose_spells(character_class):
    ...
    """
    logging.debug("Starting choose spells")
    spells = [
        "Acid Splash",
        "Air Bubble",
        "Alarm",
    ...
    for i in range(len(uppercase_list)):
        if uppercase_list[i] == choice.replace(" ", "").upper():
            index = i
            break
    logging.debug("Chosen spell: %s", spells[index])
    return [spells[index]]
...
def menu(characters):
    ...
    if choice == "4":
        ...
        logging.info("Getting information on character")
        character_details(characters)
        menu(characters)
    elif choice == "2":
        logging.info("Viewing all characters")
...
def main():
    ...
    # assert isinstance(choose_ancestry(default_stats), str), "Expecting a string"
    # assert isinstance(choose_heritage("Human"), str), "Expecting a string"
    # assert isinstance(choose_ancestry_feat("Human"), str), "Expecting a string"
    # assert isinstance(choose_class(), str), "Expecting a string"
    # assert isinstance(choose_background(), str), "Expecting a string"
    ...
    while True:
        ...
        logging.info("Character list contains %i character(s)", len(characters))
